...
模型不可用不会阻止档案记录、报告查看和医嘱管理；文件原件会先保存，AI 任务可在模型恢复后重试。Worker 升级或重启时会自动恢复尚未进入新版识别流程的文件。
...
文档长时间停在 `QUEUED` 时先检查 Worker 与 Redis，再检查 NAS 到 Mac Studio 的 5008 端口。照片在发送给模型前会自动旋转并缩放；`FAILED` 详情会保留不含报告正文的诊断信息，可从界面重新识别。不要通过降低来源校验或自动确认高风险字段来“修复”任务失败。
